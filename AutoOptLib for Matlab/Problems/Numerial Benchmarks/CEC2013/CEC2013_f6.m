function [output1,output2,output3] = CEC2013_f6(varargin)
% The f6 Function from the benchmark for the CEC 2013 Special
...
switch varargin{end}
   ...
   case 'repair'
    ...
    case 'evaluate'
        ...
        % disp(size(Decs))
        % disp(size(M))
        Decs  = (2.048*Decs/100)*M1+1;

        fit = 0;
        for i = 1:D-1
            fit = fit+100*( (Decs(:,i).^2-Decs(:,i+1)).^2 + ((Decs(:,i)-1).^2) );
        end
        output1= fit-900;
end
