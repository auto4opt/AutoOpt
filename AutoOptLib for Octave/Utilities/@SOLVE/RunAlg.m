...
for i = 1:length(Problem)
    ...
    for j = 1:Setting.AlgRuns
        switch Problem(i).type{2}
            ...
            case 'sequential'
                ...
                fitnessSequence(j) = sum(SOLVE.fits(ArchSolution));
        end        
    ...
    switch Problem(i).type{2}
        case 'static'
            [~,best] = min(SOLVE.fits(bestSolutions(i,:)));
...
elseif Setting.AlgP > 1 % if the designed algorithm has multiple search pathways
    ...
    while G <= Problem.Gmax && t < Tmax && min(SOLVE.fits(ArchSolution)) > Thres
        ...
        for i = 1:Setting.AlgP
            ...
            allNew = SOLVE.cats(allNew,New);
        end

        % update solution(s)
        [Solution,~] = feval(str2func(Operator{1}.Update),SOLVE.cats(Solution,allNew),Problem,Para{1}.Update,Aux{i},G,innerG,Data,'execute');
