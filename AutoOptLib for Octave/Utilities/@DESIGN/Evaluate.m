                ...
                for k = 1:Setting.AlgRuns
                    ...
                    switch Setting.Metric
                        case 'quality' % solution quality as performance metric
                            Performance(seedInstance(j),k) = min(SOLVE.fits(ArchSolution)); % best performance 
                        ...
                        case 'auc' % AUC as performance metric
                            ...
                            Performance(seedInstance(j),k) = 1/(sum(SOLVE.fits(ArchSolution(TimePoints))<= Setting.Thres')/numel(TimePoints)+eps); % 1/AUC, the smaller the better performance
                    end
                end
            ...
            case 'sequential'
                for k = 1:Setting.AlgRuns
                    while Data(seedInstance(j)).continue == true
                        ...
                        switch Setting.Metric
                            case 'quality'
                                Performance(seedInstance(j),k) = Performance(seedInstance(j),k) + min(SOLVE.fits(ArchSolution));
                            ...
                            case 'auc'
                                ...
                                Performance(seedInstance(j),k) = Performance(seedInstance(j),k) + 1/sum( SOLVE.fits(ArchSolution(TimePoints)) <= Setting.QuaThres)/numel(TimePoints);
                        end
                        % update the current instance of the problem sequence
                        [~,best] = min(SOLVE.fits(ArchSolution));
...
if Setting.AlgP == 1 % if the designed algorithm has a single search pathway
    while G <= Problem.Gmax && t < Tmax && min(SOLVE.fits(ArchSolution)) > Thres
        tic;
        for i = 1:size(Operator{1}.Search,1) % for each search operation
            ...
            while improve(1) >= Operator{1}.Search{i,end}(1) && innerG <= Operator{1}.Search{i,end}(2) % termination condition of search operator i
                ...
                [Solution,~] = feval(str2func(Operator{1}.Update),SOLVE.cats(Solution,New),Problem,Para{1}.Update,Aux{1},G,innerG,Data,'execute');
                ...
                if G > Problem.Gmax || t >= Tmax || min(SOLVE.fits(ArchSolution)) <= Thres
                    break
                end
            end
            if G > Problem.Gmax || t >= Tmax || min(SOLVE.fits(ArchSolution)) <= Thres
