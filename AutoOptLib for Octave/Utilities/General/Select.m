...
allPerform = zeros(numel(seedInstance)*Setting.AlgRuns,length(AllAlgs));
for i = 1:length(AllAlgs)
    allPerform(:,i) = AllAlgs(i).GetPerformance(Setting,seedInstance);
end
...
switch Setting.Evaluate
    ...
    case 'intensification'
        OldAlgs = AllAlgs(1:Setting.AlgN);
        NewAlgs = AllAlgs(Setting.AlgN+1:end);
        if strcmp(Setting.Compare,'average')
            [~,ind] = sort(c,'ascend');
            deleteInd = [];
            for i = 1:length(ind)
